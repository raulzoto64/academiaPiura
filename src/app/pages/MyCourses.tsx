    ...
    const fetchMyCourses = async () => {
      ...
      if (cachedEnrollments && cachedCatalog) {
        const mappedFromCache = cachedEnrollments.map((enrollment: any) => {
          ...
          return {
            id: courseDetails?.id || enrollment.id || enrollment.curso_id,
      ...
      try {
        ...
        const mappedCourses = myEnrollments.map((enrollment: any) => {
          // Buscar el curso en el catálogo por ID o por Título
          ...
          return {
            id: courseDetails?.id || enrollment.id || enrollment.curso_id,
            title: courseDetails?.title || enrollment.titulo,
            // Si el backend no envía progreso real, lo iniciamos en 0%
            progress: enrollment.progreso || 0,
            lastAccessed: "Recientemente", // Esto podría venir del backend futuro
